...
const SavedArticlesPage = () => {
  ...
  const fetchArticles = async () => {
    ...
    try {
      ...
      setArticles(res.data.data?.articles || res.data.articles || res.data || []);
    } catch (err) {
      console.error(err);
                ...
                >
                  ...
                  <div className="flex-1">
                    ...
                    <a 
                      ...
                      className="font-label-sm text-xs text-primary hover:underline flex items-center gap-1 truncate w-full mb-3"
                    >
                      {article.url} <span className="material-symbols-outlined text-[12px]">open_in_new</span>
                    </a>

                    <div className="flex flex-wrap gap-x-4 gap-y-1.5 py-2.5 border-y border-outline-variant/35 text-[12px] text-on-surface-variant font-medium">
                      <span className="flex items-center gap-1.5">
                        <span className="material-symbols-outlined text-[16px] text-primary">psychology</span>
                        AI Model: <strong className="text-on-surface text-[11px] bg-surface-container-low border border-outline-variant/30 rounded px-1.5 py-0.5">{article.modelUsed || 'Legacy Fallback'}</strong>
                      </span>
                      <span className="flex items-center gap-1.5">
                        <span className="material-symbols-outlined text-[16px] text-primary">fact_check</span>
                        Sources: <strong className="text-on-surface text-[11px] bg-surface-container-low border border-outline-variant/30 rounded px-1.5 py-0.5">{article.sourcesCount || 0}</strong>
                      </span>
                    </div>
                  </div>

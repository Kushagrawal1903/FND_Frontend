...
const AdminPage = () => {
  ...
  const fetchAdminData = async () => {
    ...
    try {
      ...
      setReports(reportsRes.data.data?.reports || reportsRes.data.reports || reportsRes.data || []);

      // Fetch users list
      const usersRes = await adminAPI.getUsers();
      setUsers(usersRes.data.data?.users || usersRes.data.users || usersRes.data || []);
    } catch (err) {
      console.error(err);
    ...
    <div className="bg-background min-h-screen text-on-background font-body-md flex overflow-hidden">
      ...
      <main className="flex-1 ml-0 md:ml-64 flex flex-col h-screen overflow-y-auto w-full">
        ...
        <div className="p-margin-mobile md:p-margin-desktop max-w-container-max mx-auto w-full flex flex-col gap-gutter">
          ...
          {/* AI Orchestration Metrics Grid */}
          <section className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            
            {/* Card: Gemini Requests */}
            <div className="bg-surface border border-outline-variant rounded-lg p-stack-md flex flex-col gap-2 shadow-sm">
              <div className="flex justify-between items-start">
                <span className="font-label-sm text-label-sm text-on-surface-variant uppercase tracking-wider">Gemini Requests</span>
                <span className="material-symbols-outlined text-primary text-[20px]">psychology</span>
              </div>
              <div className="font-headline-lg text-headline-lg text-on-surface font-bold">
                {stats.totals?.geminiRequests || 0}
              </div>
              <div className="text-on-surface-variant font-label-sm text-label-sm">
                Primary AI verifications
              </div>
            </div>

            {/* Card: Groq Fallback Requests */}
            <div className="bg-surface border border-outline-variant rounded-lg p-stack-md flex flex-col gap-2 shadow-sm">
              <div className="flex justify-between items-start">
                <span className="font-label-sm text-label-sm text-on-surface-variant uppercase tracking-wider">Groq Requests</span>
                <span className="material-symbols-outlined text-amber-600 text-[20px]">swap_calls</span>
              </div>
              <div className="font-headline-lg text-headline-lg text-on-surface font-bold">
                {stats.totals?.groqRequests || 0}
              </div>
              <div className="text-on-surface-variant font-label-sm text-label-sm">
                Failover AI recoveries
              </div>
            </div>
            ...
            <div className="bg-surface border border-outline-variant rounded-lg p-stack-md flex flex-col gap-2 shadow-sm">
              <div className="flex justify-between items-start">
                <span className="font-label-sm text-label-sm text-on-surface-variant uppercase tracking-wider">Cache Hits</span>
                <span className="material-symbols-outlined text-green-600 text-[20px]">electric_bolt</span>
              </div>
              <div className="font-headline-lg text-headline-lg text-on-surface font-bold">
                {stats.totals?.cacheHits || 0}
              </div>
              <div className="text-on-surface-variant font-label-sm text-label-sm">
                Sub-millisecond direct answers
              </div>
            </div>
          ...
          <section className="flex flex-col gap-3">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
              ...
              <div className="bg-surface border border-outline-variant rounded-lg p-stack-md flex flex-col gap-2 shadow-sm">
                ...
                <div className="font-headline-lg text-headline-lg text-on-surface font-bold">
                  {stats.totals?.avgResponseTime || 0} <span className="text-sm font-normal text-on-surface-variant">ms</span>
                </div>
                <div className="text-on-surface-variant font-label-sm text-label-sm">
                  Overall verification latency
                </div>
              </div>

              {/* Card: Fastest Verification */}
              <div className="bg-surface border border-outline-variant rounded-lg p-stack-md flex flex-col gap-2 shadow-sm">
                <div className="flex justify-between items-start">
                  <span className="font-label-sm text-label-sm text-on-surface-variant uppercase tracking-wider">Fastest Verification</span>
                  <span className="material-symbols-outlined text-green-600 text-[20px]">speed</span>
                </div>
                <div className="font-headline-lg text-headline-lg text-on-surface font-bold">
                  {stats.totals?.fastestVerification || 0} <span className="text-sm font-normal text-on-surface-variant">ms</span>
                </div>
                <div className="text-on-surface-variant font-label-sm text-label-sm">
                  Best case verification
                </div>
              </div>

              {/* Card: Slowest Verification */}
              <div className="bg-surface border border-outline-variant rounded-lg p-stack-md flex flex-col gap-2 shadow-sm">
                <div className="flex justify-between items-start">
                  <span className="font-label-sm text-label-sm text-on-surface-variant uppercase tracking-wider">Slowest Verification</span>
                  <span className="material-symbols-outlined text-red-600 text-[20px]">hourglass_empty</span>
                </div>
                <div className="font-headline-lg text-headline-lg text-on-surface font-bold">
                  {stats.totals?.slowestVerification || 0} <span className="text-sm font-normal text-on-surface-variant">ms</span>
                </div>
                <div className="text-on-surface-variant font-label-sm text-label-sm">
              ...
              {/* Card: Avg AI Reasoning Time */}
              <div className="bg-surface border border-outline-variant rounded-lg p-stack-md flex flex-col gap-2 shadow-sm">
                <div className="flex justify-between items-start">
                  <span className="font-label-sm text-label-sm text-on-surface-variant uppercase tracking-wider">Avg AI Reasoning</span>
                  <span className="material-symbols-outlined text-purple-600 text-[20px]">psychology</span>
                </div>
                <div className="font-headline-lg text-headline-lg text-on-surface font-bold">
                  {stats.totals?.avgLlmReasoning || 0} <span className="text-sm font-normal text-on-surface-variant">ms</span>
                </div>
                <div className="text-on-surface-variant font-label-sm text-label-sm">
                  Gemini/Groq reasoning avg
                </div>
              </div>

              {/* Card: Avg Google Fact Check Time */}
              <div className="bg-surface border border-outline-variant rounded-lg p-stack-md flex flex-col gap-2 shadow-sm">
                <div className="flex justify-between items-start">
                  <span className="font-label-sm text-label-sm text-on-surface-variant uppercase tracking-wider">Avg Google Fact Check</span>
                  <span className="material-symbols-outlined text-blue-600 text-[20px]">fact_check</span>
                </div>
                <div className="font-headline-lg text-headline-lg text-on-surface font-bold">
                  {stats.totals?.avgGoogleFactCheck || 0} <span className="text-sm font-normal text-on-surface-variant">ms</span>
                </div>
                <div className="text-on-surface-variant font-label-sm text-label-sm">
                  Google Fact Check API avg
                </div>
              </div>

              {/* Card: Avg NewsAPI Time */}
              <div className="bg-surface border border-outline-variant rounded-lg p-stack-md flex flex-col gap-2 shadow-sm">
                <div className="flex justify-between items-start">
                  <span className="font-label-sm text-label-sm text-on-surface-variant uppercase tracking-wider">Avg NewsAPI Time</span>
                  <span className="material-symbols-outlined text-amber-600 text-[20px]">newspaper</span>
                </div>
                <div className="font-headline-lg text-headline-lg text-on-surface font-bold">
                  {stats.totals?.avgNewsApi || 0} <span className="text-sm font-normal text-on-surface-variant">ms</span>
                </div>
                <div className="text-on-surface-variant font-label-sm text-label-sm">
                  NewsAPI query average
                </div>
              </div>

              {/* Card: Avg Tavily Time */}
              <div className="bg-surface border border-outline-variant rounded-lg p-stack-md flex flex-col gap-2 shadow-sm">
                <div className="flex justify-between items-start">
                  <span className="font-label-sm text-label-sm text-on-surface-variant uppercase tracking-wider">Avg Tavily Time</span>
                  <span className="material-symbols-outlined text-teal-600 text-[20px]">travel_explore</span>
                </div>
                <div className="font-headline-lg text-headline-lg text-on-surface font-bold">
                  {stats.totals?.avgTavily || 0} <span className="text-sm font-normal text-on-surface-variant">ms</span>
                </div>
                <div className="text-on-surface-variant font-label-sm text-label-sm">
                  Tavily Search API query avg
                </div>
              </div>

              {/* Card: Avg MongoDB Lookup Time */}
              <div className="bg-surface border border-outline-variant rounded-lg p-stack-md flex flex-col gap-2 shadow-sm">
                <div className="flex justify-between items-start">
                  <span className="font-label-sm text-label-sm text-on-surface-variant uppercase tracking-wider">Avg MongoDB Lookup</span>
                  <span className="material-symbols-outlined text-emerald-600 text-[20px]">database</span>
                </div>
                <div className="font-headline-lg text-headline-lg text-on-surface font-bold">
                  {stats.totals?.avgMongoHistory || 0} <span className="text-sm font-normal text-on-surface-variant">ms</span>
                </div>
                <div className="text-on-surface-variant font-label-sm text-label-sm">
                  Local claim history lookup avg
                </div>
              </div>
